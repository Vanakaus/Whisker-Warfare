...
func _ready():
	...
	set_meta("tipo", "Robot")
	life = 80
	get_node("Borrifada").position = Vector2(-1000, -1000)
...
func _process(delta):
	...
	if moving:
		...
		if fire_counter > delay:
			...
			get_node("Borrifada").position = Vector2(-26, 0)
			get_node("Borrifada").get_node('Soco').get_node('AnimatedSprite2D').play('idle')
			get_node("Borrifada").get_node('Soco').get_node('AnimatedSprite2D').play('attack')
...
func atualizaPosicao(posicao):
	...
	get_node("DetectionArea/DetectionCollision").position = Vector2(gridIncrementVector[0]/2, gridIncrementVector[1])
